...
class BaseMenuViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // we only want to do it for button on screen. I think the way to handle this is to put
        // it in this 'if let' statement so that it only gets called if the page has an item in 
        // the menuIconCollection (such as the SellerMapViewController)
        if let menuIconCollection = menuIconCollection {
